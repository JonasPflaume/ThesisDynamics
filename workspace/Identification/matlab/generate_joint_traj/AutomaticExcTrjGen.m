...
close all;
clc;
addpath /home/jiayun/git/workspace/Identification/matlab/utils /home/jiayun/git/workspace/Identification/matlab/data
...
q_min = [-2.8973   -1.7628   -2.8973   -3.0718   -2.8973   -0.0175  -2.8973]'; % [rad] 0.0175
...
q_max = [ 2.8973    1.7628    2.8973  -0.0698    2.8973    3.7525    2.8973]'; % [rad] 3.7525
extra_degree = zeros(7,1);
extra_degree(6:7) = 1;
q_max_ = q_max - ones(7,1)*deg2rad(5) - extra_degree*deg2rad(10);  % give a safety margin of 5 deg
...
Tr = 100;            % number of exciting trajectories to be generated
...
%  Verify positions, velocities and accelerations are within the limits
    if not(collide)
        ...
        if strcmp('y',str)
            ...
            fileName = strcat(path,'gen_ddq.txt');
            fileAccID = fopen(fileName,'w+');
            ...
            ddq_vrep = reshape(ddq(:,:,tr), 7*size(ddq(:,:,tr),2),1);
            ...
            % save the joint acc
            for i = 1 : size(ddq_vrep,1)
                fprintf(fileAccID,'%1.6f\n',ddq_vrep(i));
            end
            fileVelocityID = fclose(fileAccID);
